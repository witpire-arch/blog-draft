...
├── api/
│   ├── generate.js     # 초안 생성 (제목5/SEO/사진가이드/추천/태그)
│   ├── refine.js       # AI 티 제거 (본문 재윤문)
...
**초안 생성 탭 출력**
- 네이버 최적화 SEO 점수(실제 본문 분석) + 개선 체크리스트
- 제목 후보 5개 / 본문([사진N] 위치 포함) / 사진 삽입 가이드
- 이런 분들에게 추천 / 매장 정보 템플릿 / 해시태그 ~20개
- ✨ AI 티 제거 버튼 (본문 자연스럽게 재윤문)

**키워드 찾기 탭**
- 주제로 키워드 후보 추천 → 골라서 데이터랩 트렌드 그래프 확인
